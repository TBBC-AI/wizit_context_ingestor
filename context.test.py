# TODO refactor this implementation to improve library
import json
import os
...
from src.wizit_context_ingestor.infra.persistence.local_storage import (
    LocalStorageService,
)
...
def evaluate_chunk_generator(md_file_name: str, json_file_name: str):
    ...
    vertex_model = VertexModels(GCP_PROJECT_ID, GCP_PROJECT_LOCATION, gcp_sa)
    vertex_model.load_chat_model()
    chunks_and_context = []
    with open(file_key, "r") as file_json:
        chunks_and_context = json.loads(file_json.read())

    markdown_content = local_persistence_service.load_markdown_file_content(
        markdown_file_key
    )
